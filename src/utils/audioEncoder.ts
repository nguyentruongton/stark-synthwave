...
export async function exportAudioBuffer(
	...
	switch (formatId) {
		...
		case "mp3-192":
			return {
				blob: await encodeMp3(buffer, 192),
				extension: "mp3",
				mimeType: "audio/mp3",
				label: "MP3 192 kbps",
			};
...
/** Pure JavaScript MP3 Encoder via LAME (Dynamic on-demand import). */
export async function encodeMp3(
	...
	for (let i = 0; i < totalSamples; i += sampleBlockSize) {
		...
		if (chunk.length > 0) {
			mp3Data.push(chunk as unknown as BlobPart);
		}
	}

	const finalChunk = encoder.flush();
	if (finalChunk.length > 0) {
		mp3Data.push(finalChunk as unknown as BlobPart);
	}
