import { Card, Icon, IconButton, Text, useTheme } from "@bug-on/m3-expressive";
...
export function FrequencyVisualizer({
	...
	// Throttle metric updates to avoid React re-rendering at 60Hz
	...
	useEffect(() => {
		...
		// Fetch dynamic M3 colors ONCE per theme change to avoid forced layout reflows in 60fps loop
		...
		const render = () => {
			...
			if (isPlaying && analyserNode) {
				...
				const calculatedDb =
					maxVal > 0 ? Math.round((maxVal / 255) * 60 - 60) : -90;

				// Throttle React state updates to 10Hz (100ms) to prevent main thread blocking
				const now = performance.now();
				if (now - lastMetricUpdateRef.current >= 100) {
					...
					setBandEnergy({
						bass:
							bassCount > 0
								? Math.min(100, Math.round((bassSum / (bassCount * 255)) * 100))
								: 0,
						lowMid:
							lowMidCount > 0
								? Math.min(
										100,
										Math.round((lowMidSum / (lowMidCount * 255)) * 100),
									)
								: 0,
						highMid:
							highMidCount > 0
								? Math.min(
										100,
										Math.round((highMidSum / (highMidCount * 255)) * 100),
									)
								: 0,
						treble:
							trebleCount > 0
								? Math.min(
										100,
										Math.round((trebleSum / (trebleCount * 255)) * 100),
									)
								: 0,
					});
				}
				...
				if (mode === "bars") {
					...
					for (let i = 0; i < barCount; i++) {
						...
						if (barHeight >= peakCapsRef.current[i]) {
							...
							peakCapsRef.current[i] = Math.max(
								0,
								peakCapsRef.current[i] - peakCapDecayRef.current[i],
							);
						}
					...
					waveGradient.addColorStop(
						0,
						isDark ? "rgba(208, 188, 255, 0.45)" : "rgba(103, 80, 164, 0.35)",
					);
					waveGradient.addColorStop(
						0.7,
						isDark ? "rgba(239, 184, 200, 0.2)" : "rgba(125, 82, 96, 0.15)",
					);
			...
			<div className="flex items-center justify-between gap-2 flex-wrap min-w-0">
				<div className="flex items-center gap-2 min-w-0">
					<div className="w-7 h-7 rounded-lg bg-m3-primary-container flex items-center justify-center text-m3-primary shrink-0">
						<Icon
							name={isPlaying ? "graphic_eq" : "equalizer"}
							size={16}
					...
					<div className="overflow-hidden min-w-0">
						...
						>
							{language === "vi"
								? "Phổ Tần Số Thời Gian Thực (Web Audio)"
								: "Real-time Frequency Spectrum"}
						</Text>
						<Text
							variant="label-sm"
							className="text-m3-on-surface-variant text-[11px] block"
						>
							{isPlaying
								? language === "vi"
									? "Đang phân tích phản hồi âm thanh 20Hz - 20kHz"
									: "Live acoustic FFT response 20Hz - 20kHz"
								: language === "vi"
									? "Sẵn sàng (Phát nhạc để kích hoạt)"
									: "Ready (Play track to activate)"}
						</Text>
					</div>
				</div>
				...
				<div className="flex items-center gap-2">
					{showMetrics && isPlaying && peakFreq > 0 && (
					...
					<div className="flex items-center bg-m3-surface-container-high rounded-lg p-0.5 border border-m3-outline-variant/40">
						<IconButton
							...
							title={
								language === "vi"
									? "Hiển thị dạng cột quang phổ (FFT)"
									: "Spectrum Bars mode"
							}
				...
				{!isPlaying && (
					<div className="absolute inset-0 flex items-center justify-center pointer-events-none bg-m3-surface-container-lowest/25 backdrop-blur-[0.5px]">
						<span className="text-xs font-mono font-medium px-2.5 py-1 rounded-full bg-m3-surface-container-high/90 text-m3-on-surface-variant border border-m3-outline-variant/50 shadow-xs">
							{language === "vi"
								? "Nhấn Phát để xem chuyển động phổ tần số"
								: "Press Play to view real-time frequency"}
						</span>
					</div>
				)}
			</div>
				...
				<div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-0.5">
					...
					<div className="flex flex-col gap-1 p-2 rounded-lg bg-m3-surface-container-high/40 border border-m3-outline-variant/20">
						<div className="flex justify-between items-center text-[11px]">
							...
							<span className="font-mono font-bold text-m3-secondary">
								{bandEnergy.lowMid}%
							</span>
						</div>
						<div className="w-full h-1.5 bg-m3-surface-container-highest rounded-full overflow-hidden">
					...
					<div className="flex flex-col gap-1 p-2 rounded-lg bg-m3-surface-container-high/40 border border-m3-outline-variant/20">
						<div className="flex justify-between items-center text-[11px]">
							...
							<span className="font-mono font-bold text-m3-tertiary">
								{bandEnergy.highMid}%
							</span>
						</div>
						<div className="w-full h-1.5 bg-m3-surface-container-highest rounded-full overflow-hidden">
