...
import { Card, Button, IconButton, Text, Divider, LoadingIndicator, Icon, Chip } from "@bug-on/m3-expressive";
import { AudioFormatInfo, KeyResult } from "../types";
...
export function ToneDetector() {
  ...
  const getHarmonicMatches = (camelot: string) => {
    ...
    matches.push({
      ...
      relation: language === "vi" ? "Chuyển Đổi Trưởng/Thứ (Parallel)" : "Relative Major/Minor (Parallel)"
    });
    ...
    matches.push({
      ...
      relation: language === "vi" ? "Hạ tông (Subdominant)" : "1 Step Counterclockwise (Subdominant)"
    });

    const nextNum = matchNum === 12 ? 1 : matchNum + 1;
    matches.push({
      key: `${nextNum}${matchLetter}`,
      relation: language === "vi" ? "Tăng tông (Dominant)" : "1 Step Clockwise (Dominant)"
    });
  ...
  return (
    <div className="flex flex-col gap-6 overflow-hidden" id="tone-tab">
      <Card variant="outlined" className="p-4 sm:p-6 flex flex-col gap-5 overflow-hidden bg-m3-surface-container-lowest/40 rounded-2xl border-m3-outline-variant/40">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 min-w-0">
          <div className="flex items-start sm:items-center gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <Text variant="title-md" className="font-semibold text-m3-on-surface wrap-break-word whitespace-normal">{t("tone_title")}</Text>
              <Text variant="body-sm" className="text-m3-on-surface-variant wrap-break-word whitespace-normal">{t("tone_desc")}</Text>
            </div>
          </div>
        ...
        {showInfo && (
          <Card variant="filled" className="bg-m3-primary-container/25 border border-m3-primary/30 p-4 rounded-xl flex flex-col gap-2">
            <Text variant="body-md" className="font-semibold text-m3-on-primary-container wrap-break-word whitespace-normal">{t("tone_help_title")}</Text>
        ...
        {error && (
          <Card variant="filled" className="flex items-center justify-between p-4 bg-m3-error-container text-m3-on-error-container border border-m3-error/30 rounded-xl animate-in fade-in duration-200 min-w-0 gap-2">
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <Icon name="error" className="text-m3-error shrink-0" />
              <Text variant="body-sm" className="text-m3-on-error-container font-medium wrap-break-word whitespace-normal min-w-0 flex-1">{error}</Text>
            </div>
            <IconButton colorStyle="standard" onClick={() => setError(null)} aria-label="Close error" className="shrink-0">
        ...
        {!file ? (
          ...
          >
            <div
              className="w-16 h-16 rounded-full bg-m3-primary-container/40 flex items-center justify-center text-m3-primary cursor-pointer hover:scale-105 transition-transform"
            ...
            <div
              ...
              onClick={() => document.getElementById("tone-upload")?.click()}
            >
              <Text variant="title-md" className="font-semibold text-m3-on-surface wrap-break-word whitespace-normal">{t("tone_drop_title")}</Text>
              <Text variant="body-sm" className="text-m3-on-surface-variant wrap-break-word whitespace-normal mt-1">{t("tone_drop_desc")}</Text>
            </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <Card variant="filled" className="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 bg-m3-surface-container-low rounded-xl gap-3 border border-m3-outline-variant/40">
              <div className="flex items-center gap-3 overflow-hidden min-w-0 flex-1">
                ...
                <div className="overflow-hidden min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Text variant="body-md" className="font-semibold text-m3-on-surface truncate block">
                      {file.name}
                    </Text>
                    {detectedFormat && (
                      <Chip
                        variant="assist"
                        leadingIcon={<Icon name={detectedFormat.isLossless ? "verified" : "audiotrack"} size={14} />}
        ...
        {isLoading && (
          <Card variant="outlined" className="flex flex-col items-center justify-center gap-3.5 p-8 sm:p-10 bg-m3-surface-container-low/40 rounded-2xl border-m3-outline-variant/30 text-center">
            <LoadingIndicator aria-label="Analyzing song key" size={48} className="shrink-0" />
            <Text variant="title-sm" className="text-m3-primary font-semibold">{t("tone_analyzing")}</Text>
            <Text variant="body-sm" className="text-m3-on-surface-variant max-w-md">
              {language === "vi" ? "Đang phân tích cấu trúc phổ hòa âm và tần số các nốt nhạc..." : "Analyzing harmonic energy spectrum and chroma note frequencies..."}
            </Text>
          </Card>
        )}
        ...
        {result && (
          <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {/* Core Key Panel */}
              <Card variant="filled" className="p-4 sm:p-6 bg-m3-primary-container/25 border border-m3-primary/30 rounded-2xl flex flex-col items-center justify-center text-center gap-2">
                <Text variant="body-sm" className="text-m3-on-surface-variant uppercase tracking-wider font-semibold text-xs">{t("tone_result_label")}</Text>
                <Text variant="display-md" className="font-extrabold text-m3-primary mt-1 text-3xl sm:text-5xl md:text-6xl font-mono">
                  {result.keyName}
                </Text>
                <div className="flex flex-wrap items-center justify-center gap-2 mt-2">
                  ...
                  <Chip
                    variant="assist"
                    elevated
                    leadingIcon={<Icon name={result.confidence > 75 ? "verified" : "speed"} size={16} />}
              ...
              <Card variant="filled" className="p-4 sm:p-5 bg-m3-surface-container-high rounded-2xl flex flex-col gap-3 border border-m3-outline-variant/30">
                <Text variant="title-sm" className="font-semibold text-m3-on-surface">{t("tone_harmonic_title")}</Text>
                  ...
                  {getHarmonicMatches(result.camelot).map(match => (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        ...
                        <Text variant="body-md" className="text-m3-on-surface-variant text-xs font-medium wrap-break-word whitespace-normal leading-tight">
                          {match.relation}
                        </Text>
                      </div>
                      <Chip
            ...
            <div className="flex flex-col gap-3">
              <Text variant="title-sm" className="font-semibold text-m3-on-surface">{t("tone_chroma_title")}</Text>
                        ...
                        <div className="w-full bg-m3-surface-container-highest rounded-md h-7.5 sm:h-10 flex items-end overflow-hidden">
                          <div
                            className={`w-full rounded-t-sm transition-all duration-500 ${
                              isDetectedNote ? "bg-m3-primary" : "bg-m3-outline-variant"
                            }`}
                        ...
                        <div className="text-center min-w-0">
                          <Text
                            ...
                            className={`font-bold text-[10px] sm:text-xs leading-none ${
                              isDetectedNote ? "text-m3-primary" : "text-m3-on-surface"
                            }`}
                          >
                            {NOTE_LABELS[idx]}
                          </Text>
                          <Text
                            ...
                            className={`text-[8px] sm:text-[9px] block font-mono mt-0.5 ${
                              isDetectedNote ? "text-m3-primary font-bold" : "text-m3-on-surface-variant"
                            }`}
                          >
                            {(energy * 10).toFixed(1)}
                          </Text>
              ...
              <Text variant="body-sm" className="text-m3-on-surface-variant text-center mt-1 wrap-break-word whitespace-normal">{language === "vi" ? "Các nốt có mức năng lượng cao nhất quyết định cấu trúc giọng và thang âm của toàn bài hát." : "Notes with the highest harmonic energy determine the song's root key and tonality."}</Text>
            </div>
          </div>
        )}
      </Card>
