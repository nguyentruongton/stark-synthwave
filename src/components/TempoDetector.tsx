...
import type { AudioFormatInfo, BPMResult } from "../types";
...
export function TempoDetector() {
	...
	const processAndDetectBPM = async (selectedFile: File) => {
		...
		try {
			...
			if (worker) {
				const bpmResult = await new Promise<BPMResult>((resolve, reject) => {
			...
			>
				...
				{isLoading && (
					...
					>
						...
						>
							{language === "vi"
								? "Đang phân tích phổ âm thanh và các đỉnh nhịp điệu trên máy của bạn..."
								: "Analyzing audio spectrum and rhythmic energy peaks locally..."}
						</Text>
					</Card>
				)}
				...
				{result && (
					<Card
						variant="filled"
						className="flex flex-col items-center gap-5 p-6 bg-m3-primary-container/25 border border-m3-primary/30 rounded-2xl animate-in fade-in slide-in-from-bottom-2 duration-300"
					>
						<div className="text-center">
							<Text
								variant="body-sm"
								className="text-m3-on-surface-variant uppercase tracking-wider font-semibold"
							>
								{t("tempo_result_label")}
							</Text>
							<div className="flex items-baseline justify-center gap-1.5 mt-1">
								<Text
									variant="display-lg"
									className="font-extrabold text-m3-primary font-mono tracking-tight text-5xl sm:text-6xl"
								>
									{result.bpm}
								</Text>
								<Text
									variant="title-md"
									className="text-m3-on-surface-variant font-semibold"
								>
									BPM
								</Text>
							</div>
							<div className="flex items-center justify-center gap-2.5 mt-2.5 flex-wrap">
								...
								<Text
									variant="body-sm"
									className="text-m3-on-surface-variant font-medium"
								>
									{language === "vi"
										? `Phát hiện ${result.peaksCount} điểm nhấn nhịp`
										: `${result.peaksCount} rhythmic peaks`}
								</Text>
							</div>
						</div>
						...
						<div className="flex flex-col items-center gap-2 mt-1">
							...
							>
								<Icon
									name="favorite"
									fill={pulseActive ? 1 : 0}
									size={28}
									className="text-m3-primary transition-all"
								/>
							</div>
							<Text
								variant="body-sm"
								className="text-m3-on-surface-variant text-center font-mono text-xs"
							>
								{language === "vi"
									? "Nhịp tim đập theo nhịp điệu của bài hát"
									: "Pulsing to detected track tempo"}
							</Text>
						</div>
					</Card>
				)}
						...
						{tapBPM ? (
							<div className="animate-in zoom-in-95 duration-100">
								...
								<div className="flex items-baseline justify-center gap-1.5 mt-0.5">
									<Text
										variant="display-lg"
										className="font-extrabold text-m3-secondary font-mono text-5xl sm:text-6xl"
									>
										{tapBPM}
									</Text>
									<Text
							...
							<div className="flex flex-col items-center gap-1">
								<Text
									variant="body-md"
									className="text-m3-on-surface-variant italic font-medium"
								>
									{language === "vi"
										? "Nhấn phím hoặc chạm nút bên dưới theo nhịp..."
										: "Click or tap button rhythmically..."}
								</Text>
								<Text
									variant="body-sm"
									className="text-m3-on-surface-variant/80 text-xs"
								>
									{language === "vi"
										? "Hãy đệm từ 4 nhịp trở lên liên tục"
										: "Tap at least 4 times consistently"}
								</Text>
							</div>
						)}
					</div>
					...
					>
						<div
							className={`w-12 h-12 rounded-full flex items-center justify-center transition-transform duration-150 ${
								pulseActive && tapBPM
									? "scale-110 bg-m3-secondary text-m3-on-secondary"
									: "bg-m3-secondary-container/50 text-m3-secondary"
							}`}
						>
							<Icon
								name="album"
								size={28}
								className={pulseActive && tapBPM ? "rotate-45" : ""}
							/>
						</div>
						...
						<Text
							variant="body-sm"
							className="text-m3-on-surface-variant text-xs font-mono"
						>
							{tapTimes.length > 0
								? language === "vi"
									? `Đã đệm ${tapTimes.length} lần`
									: `${tapTimes.length} taps recorded`
								: language === "vi"
									? "Chạm hoặc đệm theo nhịp"
									: "Tap to Beat"}
						</Text>
					</Card>
